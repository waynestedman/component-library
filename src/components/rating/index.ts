...
export class CaseRating extends LitElement {
  ...
  headerTemplate() {
    return html`<h2>${this.starRating.title}</h2>`;
  }
  ...
  ratingTemplate() {
    return html`
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id="grad">
            <stop offset=${this.starFill.half} stop-color="var(--star-yellow)" stop-opacity="${this.starFill.solid}"/>
      ...
      <caption>${this.starRating.rating}</caption>
    `
  }
